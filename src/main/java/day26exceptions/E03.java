...
public class E03 {
    ...
    /**
     *
     1) "FileNotFoundException" is related with the "Path of the file" and the
     ...
     7)When you encounter a "Compile Tome Exception", you have 2 options
        i)Using "throws" in method signature line
            *Java will throw exception and stop the execution
        ii)Using try-catch block
            *Java will execute whatever you typed inside the catch body and
            does not stop the execution.

     8) COMMON INTERVIEW QUESTION: What is the difference between "throw" and "throws" keywords?
     ...
     (HeapMemory) OutOfMemory, StackOutOfMemory, VirtualMachine, Assertion can be example for errors.
     StackOverFlowError
    ...
    public static void readTxt(){
        ...
        try {
            FileInputStream fis = new FileInputStream("srcx/main/java/day26exceptions/FileTxt");
            int k = 0;
            while ((k = fis.read()) !=-1){
                System.out.print((char)k);
            }
